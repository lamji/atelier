 ...
 *
 * Atelier's code guards exist to enforce its own conventions — keep one
 * function per file, lay UI out with flex, patch instead of rewriting. A
 * task the user explicitly asked to run as a plain Claude/Codex turn has no
 * knowledge engine behind it, so those guards would block edits the run has
 * no way to unblock. They consult this registry and step aside.
 *
